...
export const handleAIPrompt = async (
  ...
  if (config.provider === "openai") {
    if (!config.apiKey)
      throw new Error("OpenAI API Key is required in node configuration.");
    ...
    if (!config.apiKey)
      throw new Error("Anthropic API Key is required in node configuration.");
    ...
    if (!config.apiKey)
      throw new Error(
        "Google Gemini API Key is required in node configuration.",
      );
